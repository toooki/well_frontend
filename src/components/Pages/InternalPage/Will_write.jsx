	...
	const renderWillContent = () => {
		if (mode === "text") {
			...
			return (
				<audio controls>
					<source
						src={`data:audio/webm;base64,${savedWill}`}
						type="audio/webm"
					/>
	...
	return (
		<div className="Will_Write_container">
			<div className="top-bar">
				<button className="Will_Write_main" onClick={goToMenu}>
					메뉴
				</button>
			</div>
			<h1>유언장</h1>
			{mode === "" ? (
				<div className="mode-buttons">
					...
					<button className="mode-button" onClick={() => setMode("audio")}>
						음성으로 작성
					</button>
				</div>
			) : (
					...
					{mode === "text" && (
						<div className="text-input">
							...
							<div className="action-buttons">
								<button className="action-button" onClick={saveTextWill}>
									유언장 저장
								</button>
								<button className="action-button" onClick={resetMode}>
									모드 변경
								</button>
							</div>
						</div>
					...
					{mode === "audio" && (
						<div className="audio-controls">
							...
							{audioBlob && (
								<div>
									<div className="action-buttons">
										<button className="action-button" onClick={saveAudioWill}>
											유언장 저장
										</button>
										<button className="action-button" onClick={resetMode}>
											모드 변경
