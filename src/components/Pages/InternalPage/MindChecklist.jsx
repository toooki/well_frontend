...
function SavetheList({ formData }) {
  const handleSavemindlist = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:8080/savemindlist', formData);
  ...
  return (
    <button className="Mind_checklist_save" onClick={handleSavemindlist}>저장</button>
  );
}
...
function MindChecklist({ goToMenu, username, password }) {
  ...
  const fetchUsermindList = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/mindlistuser?username=${username}`);
  ...
  const handleDelete = (index) => {
    ...
    const updatedFormData = {
      ...formData,
      mindList: updatedItems.map(item => ({ text: item.text, completed: item.completed }))
    };
  ...
  return (
    <div className="Mind_checklist_Container">
      <div className="top-bar">
        <button className="Mind_checklist_main" onClick={() => goToMenu()}>메뉴</button>
        <SavetheList formData={formData} />
      </div>
      <h1>마음의 빚 청산</h1>
      <h4>마음에 담겨있는 말, 돌려주지 않은 물건, 하지 못한 말 등을 적어주세요.</h4>
      <form onSubmit={handleSubmit}>
        <input type="text" className='Mind_checklist_set' value={text} onChange={handleChange} placeholder="예: 코딩 어렵다." />
        <button type="submit" className="Mind_checklist_add">추가</button>
      </form>
      <ul>
