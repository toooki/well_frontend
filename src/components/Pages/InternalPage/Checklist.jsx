...
function SavetheList({ formData }) {
  const handleSaveBucketlist = async () => {
    try {
      const response = await axios.post('http://localhost:8080/savebuckitlist', formData);
  ...
  return (
    <button onClick={handleSaveBucketlist} className="bucketlist-save">저장</button>
  );
}
...
function Checklist({ goToMenu, username, password }) {
  ...
  const fetchUserBucketList = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/bucketlistuser?username=${username}`);
  ...
  const handleDelete = (index) => {
    ...
    const updatedFormData = {
      ...formData,
      bucketList: updatedItems.map(item => ({ text: item.text, completed: item.completed }))
    };
  ...
  return (
    <div className="bucketlistContainer">
      <div className="top-bar">
        <button onClick={() => goToMenu()} className="bucketlist-main">메인</button>
        <SavetheList formData={formData} />
      </div>
      <h2>버킷리스트</h2>
      <form onSubmit={handleSubmit} className='bucket_list_form'>
        <input type="text" className="bucketlist-text" value={text} onChange={handleChange} placeholder="예: 나는 주식 대박나고 싶다." />
        <button type="submit" className="bucketlist-add">추가</button>
      </form>
      <ul>
