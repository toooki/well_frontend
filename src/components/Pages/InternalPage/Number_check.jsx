...
function SavetheList({ formData }) {
  const handleSavemindlist = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:8080/savephonenumber', formData);
  ...
  return (
    <button className='Phone_number_save' onClick={handleSavemindlist}>저장</button>
  );
}
...
const Number_check = ({ username, password, goToMenu }) => {
  ...
  const fetchUsermindList = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/phonenumberuser?username=${username}`);
  ...
  const isPhoneNumberValid = (number) => {
    ...
    const phoneNumberPattern = /^01[0-9]{1}-[0-9]{4}-[0-9]{4}$|^01[0-9]{1}[0-9]{4}[0-9]{4}$/;
  ...
  const handleSubmit = (e) => {
    ...
    if (!isPhoneNumberValid(phoneNumber)) {
      setError('전화번호 형식이 올바르지 않습니다. 형식: 010-1234-5678 또는 01012345678');
      return;
    } else {
      setError('');
    }
    ...
    // 새로운 아이템 생성
    const newItem = { phonename: name.trim(), phonenumber: formattedPhoneNumber };
  ...
  return (
    <div className="phoneListContainer">
      <div className="top-bar">
        <button className="Phone_number_main" onClick={() => goToMenu()}>메뉴</button>
      ...
      <form onSubmit={handleSubmit} className='phone_Number_form'>
        ...
        />
        <button type="submit" className="phone_Number_add">추가</button>
      </form>
      {error && <p className="error">{error}</p>}
      <ul>
        {items.map((item, index) => (
          <li key={index}>
            <span>{item.phonename} {item.phonenumber}</span>
            <button className="handleDelete" onClick={() => handleDelete(index)}>X</button>
          </li>
        ))}
      </ul>
    </div>
