...
const services = {
  ...
  '프리드라이프': 'https://preed-life.co.kr/?gad_source=1&gclid=CjwKCAjw2dG1BhB4EiwA998cqNeqtNnTiXxLuP_lxHBtKahxn13vTmhLOgxO4Mm__QExNVOYIOmPfxoC3r0QAvD_BwE',
...
const Funeral_planning = ({ username, password, goToMenu }) => {
  ...
  const [selectedHospital, setSelectedHospital] = useState('선택한 병원이 없습니다.');
  const [selectedFuneralHome, setSelectedFuneralHome] = useState('선택한 장례식장이 없습니다.');
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/load/${username}`);
        const data = response.data;

        setOrganDonation(data.organDonation || '');
        setBodyDonation(data.bodyDonation || '');
        setBurialOrCremation(data.burialOrCremation || '');
        setSelectedHospital(data.hospital ? `이송희망병원: ${data.hospital}` : '선택한 병원이 없습니다.');
        setSelectedFuneralHome(data.funeralHome ? `선택한 장례식장: ${data.funeralHome}` : '선택한 장례식장이 없습니다.');
        setSelectedService(data.funeralServiceAgencies || '');
        setPronunciationFile(data.pronunciationName ? new File([data.pronunciation], data.pronunciationName) : null);
        setImage(data.pronunciation ? `data:image/jpeg;base64,${data.pronunciation}` : null);
  ...
  const handleSave = async () => {
    if (!organDonation || !bodyDonation || !burialOrCremation || !selectedHospital || !selectedService || !selectedFuneralHome) {
    ...
    formData.append('funeralHome', selectedFuneralHome.replace('선택한 장례식장: ', ''));
  ...
  return (
    <div className='Funeral_planning_container'>
      <div className="top-bar">
        <button className='Funeral_planning_save' onClick={handleSave}>저장</button>
        <button className='Funeral_planning_main' onClick={() => goToMenu()}>메뉴</button>
      </div>
      <h1>장례계획</h1>
      <p>장례의사 묻기(장기기증, 시신기증의사 장지(매장or화장)에 관한 희망사항)</p>
      ...
      <p>이송할 병원 알아두기</p>
      <iframe src="/Funeral_planning.html" width="100%" height="500px" title="Searching Hospital"></iframe>
      ...
      <div>
        {Object.keys(services).map((service) => (
          <div key={service}>
            ...
            <button className='Funeral_planning_homepage' onClick={() => openServiceWebsite(services[service])}>
              홈페이지 방문
            </button>
          </div>
        ))}
      ...
      <p>장례식장 정하기</p>
      <iframe src="/Search_funeral.html" width="100%" height="500px" title="Searching Funeral Home"></iframe>
