...
function SavetheList({ formData }) {
	...
	return (
		<button onClick={handleSaveHealthData} className="save-button_checklist">
			저장
		</button>
	);
}
...
const HealthChecklist = ({ goToMenu, username, password }) => {
	const questions1 = [
		{
			id: "1-1",
			question:
				"1. 하루 30분이상 근력 운동을 하십니까? (단순 걷기는 포함하지 않음)",
			options: ["주 3회 이상", "주 1-2회", "안함"],
		},
		{
			id: "1-2",
			question:
				"2. 하루 30분이상 유산소 운동을 하십니까? 예) 걷기, 자전거타기, 스트레칭",
			options: ["주 3회 이상", "주 1-2회", "안함"],
		},
		{
			id: "1-3",
			question:
				"3. 보조기구나 타인의 도움 없이 15분 이상 쉬지 않고 걸을 수 있습니까?",
			options: ["항상", "보통", "아니다"],
		},
		{
			id: "1-4",
			question:
				"4. 평균 하루 물 섭취량은 어떻게 되십니까? 예) 1잔은 일반 컵 300ml 기준",
			options: ["1~2잔", "3-4잔", "5-6잔", "7잔 이상"],
		},
		{
			id: "1-5",
			question:
				"5. 카페인이 함유된 음료의 하루 평균 섭취량은 어떻게 되십니까? 예) 커피, 차, 콜라",
			options: ["안 마심", "1~2잔", "3-4잔", "5-6잔", "7잔 이상"],
		},
		{
			id: "1-6",
			question:
				"6. 달거나 짠 음식, 동물성 지방이 많은 음식을 자주 섭취 하십니까? 예) 돼지고기, 기름기 많은 음식",
		...
		},
		{
			id: "1-9",
			question:
				"9. 질병으로 처방받은 복용 약의 개수는 몇 개 십니까? 예) 고혈압, 관절염, 고지혈증, 당뇨약 등",
	...
	const questions2 = [
		...
		},
		{
			id: "2-12",
			question:
				"12. 당신은 가게에서 사려고 하는 두, 세가지 물건 이름을 기억하기 어렵습니까?",
			options: ["네", "아니요"],
		},
		{
			id: "2-13",
	...
	const questions3 = [
		...
		},
		{
			id: "3-3",
			question:
				"3. 신경이 예민해지고 스트레스를 받고 있다는 느낌을 경험하지 않았던 적이 있었다.",
			options: [
				"전혀 없었다",
				"거의 없었다",
				"때때로 있었다",
		...
		},
		{
			id: "3-10",
			question:
				"10. 어려운 일들이 많이 쌓여 해결하지 못할 것 같은 느낌을 경험했다.",
			options: [
				"전혀 없었다",
				"거의 없었다",
				"때때로 있었다",
	...
	const questions4 = [
		{
			id: "4-1",
			question:
				"1. 나는 질병을 가지고 있거나, 먹는 음식의 양이나 종류를 변화시키는 상태이다.",
			options: ["네", "아니요"],
		},
		{
			id: "4-2",
		...
		},
		{
			id: "4-5",
			question:
				"5. 나는 치아와 구강문제를 가지고 있어서 먹는데 어려움을 느낀다.",
			options: ["네", "아니요"],
		},
		{
			id: "4-6",
		...
		return (
			<div className="HealthChecklist_container">
				<div className="top-bar">
					<button
						type="button"
						className="HealthChecklist_main"
						onClick={() => goToMenu()}
					>
						메뉴
					</button>
					<button
						type="button"
						className="HealthChecklist_save"
